import re import asyncio import logging import os import threading from datetime import datetime

from flask import Flask from telethon import TelegramClient, events, Button from telethon.tl.types import ChannelParticipantAdmin, ChannelParticipantCreator

===== CONFIGURATION =====

api_id = 28885685  # Your API ID api_hash = 'c24e850a947c003557f614d6b34035d9'  # Your API hash user_session = 'gobble'  # Using the user session bot_session = 'wobble'   # Bot client session name bot_token = '7886946660:AAGXvcV7FS5uFduFUVGGzwwWg1kfua_Pzco'  # Your Bot Token

Channel details:

SOURCE_CHANNEL_ID = -1001998961899  # Source channel (@gem_tools_calls) TARGET_CHANNEL_ID = -1002405509240   # Target channel (Wagmi Vip ☢️) TARGET_CHANNEL_TITLE = "Wagmi Vip ☢️"  # For logging purposes
...
@app.route('/') def index(): return "Service is running", 200

def run_flask(): port = int(os.environ.get("PORT", 5000)) # Run on all interfaces so that Render can access the service. app.run(host='0.0.0.0', port=port)

===== HELPER FUNCTIONS =====

def extract_contract(text: str) -> str | None: """ Extracts a Solana contract address. """ pattern = r"Contract Address:\s*\n\s*([A-Za-z0-9]{32,50})" match = re.search(pattern, text, re.IGNORECASE) if match: return match.group(1).strip() match = re.search(r"Ca\s*:\s*([A-Za-z0-9]{32,50})", text, re.IGNORECASE) if match: return match.group(1).strip() tokens = re.findall(r'\b[A-Za-z0-9]{32,50}\b', text) return tokens[0].strip() if tokens else None

def parse_tff_output(text: str) -> dict: """ Parses the output from TFF bot and returns a dictionary with: token_name, market_cap, mint_status, and liquidity_status. Handles messages starting with both "📌" and "💊". """ data = {} # Try to extract token name using the "📌" pattern first. m = re.search(r"📌\s*([^\n⚠]+)", text) if m: token_name = m.group(1).strip() else: # Fallback for messages starting with "💊": extract the first word after the emoji. m = re.search(r"💊\s*([^\s(]+)", text) token_name = m.group(1).strip() if m else "unknown token" # Remove any trailing parentheses and their contents. token_name = re.sub(r"\s.?", "", token_name).strip() data["token_name"] = token_name.lower()
...
def build_new_template(token_name: str, contract: str, market_cap: str, liquidity_status: str, mint_status: str, chart_url: str) -> str: """ Builds the new announcement message template. """ template = ( "🚀 New 💎 GEM Just Landed! 🚀\n\n" f"💰 {token_name.capitalize()}– Ape in Before Liftoff!\n\n" "💬 "Degens move fast. Follow my lead, and wealth will follow. I have paved the way before—and I will do it again."\n\n" f"📊 Market Cap: {market_cap}\n" f"💦 Liquidity: {liquidity_status}\n" f"🔥 Minting: {mint_status}\n\n" f"🔗 Contract: {contract}\n" "🌐 Network: #SOL\n\n" "🔥 100x or dust—are you printing or fading? 🔥\n" "🔹 Real-time chart tracking to keep you ahead!\n" "🔹 Maximum hype—early apes prosper first!\n\n" "⚔️ Ape in & send this 💎 GEM to Valhalla! ⚔️\n" "📢 Tag the squad and let’s all rise to riches together.\n\n" "⚠️ Degens, remember:\n" "🔸 Always secure your profits—don’t hold on to bags too long.\n" "🔸 Ride the wave, but never let greed overtake you.\n" "🔸 DYOR & only invest what you can afford to lose." ) return template

def build_inline_buttons(tff_data: dict) -> list: """ Builds inline buttons for the initial announcement message. """ contract = tff_data.get("contract", "") row1 = [ Button.url("View Chart", f"https://dexscreener.com/solana/{contract}"), Button.url("Trojan", f"https://t.me/solana_trojanbot?start=r-ttf-{contract}") ] row2 = [ Button.url("Bit Foot", f"https://t.me/BitFootBot?start=buy={contract}Solana_ttfbot"), Button.url("MEVX", f"https://t.me/MevxTradingBot?start={contract}") ] row3 = [ Button.url("Algora", f"https://t.me/algoratradingbot?start=r-tff-{contract}") ] row4 = [ Button.url("GMGN", f"https://t.me/GMGN_sol03_bot?start=i_30I510nA_c{contract}"), Button.url("Nova", f"https://t.me/TradeonNovaBot?start=r-VF5CM4TB-{contract}") ] return [row1, row2, row3, row4]

async def get_tff_data(contract: str) -> dict: """ Registers the handler before sending the contract, then sends the contract to the TFF bot and waits for its detailed response. """ loop = asyncio.get_event_loop() future = loop.create_future()
...
@user_client.on(events.NewMessage(chats=SOURCE_CHANNEL_ID)) async def message_handler(event): text = event.raw_text logger.info(f"Received message from source channel:\n{text}")
...
@user_client.on(events.NewMessage(chats=SOURCE_CHANNEL_ID)) async def token_update_handler(event): text = event.raw_text logger.info(f"Received update message:\n{text}")
...
async def main(): await user_client.start() await bot_client.start(bot_token=bot_token) logger.info("User client (gobble) and Bot client (wobble) are running...")

# Run the user client until disconnected.
await user_client.run_until_disconnected()

if name == "main": # Start the Flask web server in a background thread so Render detects a web service. flask_thread = threading.Thread(target=run_flask) flask_thread.daemon = True flask_thread.start()
