...
def parse_tff_output(text: str) -> dict:
    ...
    logger.debug("✅ Parsed TTF output.")
    return data
...
async def login():
    if request.method == 'POST':
        ...
        try:
            ...
            logger.info(f"➡️ Sent login code request to {phone}")
...
async def admin_callback_handler(event):
    ...
    try:
        ...
        if data == 'admin_pause':
            ...
            return await event.edit("⏸ *Pause Bot*\n\nHow many minutes should I pause for?", buttons=kb, link_preview=False)
        if data == 'admin_stop':
            await set_bot_setting("bot_status", "stopped")
            await event.answer('🛑 Bot stopped')
            return await event.edit("🛑 *Bot has been shut down.*", buttons=[[Button.inline("🔄 Restart Bot (set to running)", b"admin_start")], [Button.inline("🔙 Back", b"admin_home")]], link_preview=False)
        if data == 'admin_admins':
            ...
            if removable_admins:
                 kb.append([Button.inline("🗑️ Remove Admin", b"admin_show_remove_admins")])
        ...
        if data == 'admin_show_remove_admins':
            ...
            if not removable_admins:
                 return await event.edit("🗑️ *No more removable admins.*", buttons=[[Button.inline("🔙 Back", b"admin_admins")]], link_preview=False)
            else:
                 return await event.edit("🗑️ *Select Admin to Remove*", buttons=kb, link_preview=False)

        if data == 'admin_add_admin':
            pending_input[uid] = {'action': 'confirm_add_admin'}
            return await event.edit("➕ *Add Admin*\n\nSend me the user ID to add:", buttons=[[Button.inline("🔙 Back", b"admin_admins")]], link_preview=False)
        ...
        if data == 'admin_targets':
            ...
            if targets:
                 kb.append([Button.inline("🗑️ Remove Target", b"admin_show_remove_targets")])
            kb.append([Button.inline("🔙 Back", b"admin_home")])
        ...
        if data == 'admin_add_target':
            ...
            return await event.edit("➕ *Add Target Channel*\n\nSend me the channel ID (e.g., `-1001234567890`) or @username to add:", buttons=[[Button.inline("🔙 Back", b"admin_targets")]], link_preview=False)
        if data.startswith('remove_target:'):
            ...
            kb.append([Button.inline("🔙 Back", b"admin_targets")])
            if not targets:
                 return await event.edit("🗑️ *No more target channels.*", buttons=[[Button.inline("🔙 Back", b"admin_targets")]], link_preview=False)
            else:
                 return await event.edit("🗑️ *Select Target Channel to Remove*", buttons=kb, link_preview=False)
        ...
        if data == 'admin_sources':
            ...
            if sources:
                 kb.append([Button.inline("🗑️ Remove Source", b"admin_show_remove_sources")])
            kb.append([Button.inline("🔙 Back", b"admin_home")])
        ...
        if data == 'admin_show_remove_sources':
             ...
             return await event.edit("🗑️ *Select Source Channel to Remove*", buttons=kb, link_preview=False)
        if data == 'admin_add_source':
            pending_input[uid] = {'action': 'confirm_add_source'}
            return await event.edit("➕ *Add Source Channel*\n\nSend me the channel ID (e.g., `-1001234567890`) or @username to add:", buttons=[[Button.inline("🔙 Back", b"admin_sources")]], link_preview=False)
        if data.startswith('remove_source:'):
            ...
            kb.append([Button.inline("🔙 Back", b"admin_sources")])
            if not sources:
                 return await event.edit("🗑️ *No more source channels.*", buttons=[[Button.inline("🔙 Back", b"admin_sources")]], link_preview=False)
            else:
                 return await event.edit("🗑️ *Select Source Channel to Remove*", buttons=kb, link_preview=False)

        if data == 'admin_update_gif':
            pending_input[uid] = {'action': 'confirm_update_gif'}
            return await event.edit("🎬 *Update GIF*\n\nSend me the new GIF URL:", buttons=[[Button.inline("🔙 Back", b"admin_home")]], link_preview=False)

    except Exception as e:
         logger.error(f"Error in admin callback handler for user {uid}, data {data}: {e}")
         await event.answer("An error occurred.", alert=True)
         try:
             await event.edit(await get_admin_dashboard(), buttons=build_admin_keyboard(), link_preview=False)
         except Exception:
              pass
        ...
        try:
            if act == 'pause':
                try:
                    ...
                    await event.reply("⚠️ Please send a valid positive number of minutes.")
                    pending_input[uid] = {'action': 'pause'}
                    return
                await set_bot_setting("bot_status", "paused")
                await event.reply(f"⏸️ Paused for {m} minutes.")
            ...
            if act == 'confirm_add_admin':
                try:
                    ...
                    await event.reply("⚠️ Invalid user ID. Please send a positive integer ID.")
                    pending_input[uid] = {'action': 'confirm_add_admin'}
                    return
                current_admins = await get_admins()
                if new_id in current_admins:
                    await event.reply(f"ℹ️ User ID {new_id} is already an admin.")
                else:
            ...
            if act == 'confirm_update_gif':
                ...
                if not link.startswith(('http://', 'https://')):
                    await event.reply("⚠️ Invalid URL format. Please send a valid HTTP or HTTPS URL.")
                    pending_input[uid] = {'action': 'confirm_update_gif'}
                    return
            ...
            await event.reply("⚠️ An unexpected error occurred while processing your input.")
...
async def channel_handler(event):
        ...
        for target_channel_info in target_channels:
            ...
            try:
                ...
                await retry_telethon_call(bot_client.send_file(
                    ...
                    buttons=[[Button.url("🔗 Don't Miss Out", f"https://t.me/solana_trojanbot?start=r-ttf-{contract_for_button}")]]
                ))
    ...
    try:
        ...
        if not await user_client.is_user_authorized():
            logger.warning("⚠️ User client not authorized. TTF bot interaction skipped for new call.")
            return
        ...
        async with user_client.conversation('@ttfbotbot', timeout=90) as conv:
            ...
            logger.info(f"⬅️ Received response from @ttfbotbot (Msg ID: {ttf_response.id}) for message {message_id}.")

    except asyncio.TimeoutError:
        logger.warning(f"⚠️ TTF bot conversation timed out for contract {contract} from message {message_id}.")
        await retry_telethon_call(bot_client.send_message(DEFAULT_ADMIN_ID, f"⚠️ TTF bot timed out for contract: `{contract}` (from message {message_id} in {chat_id})."))
        return
    ...
    if ttf_response and ttf_response.raw_text:
        ...
        new_text = build_new_template(token_name, contract, data.get('market_cap', 'N/A'), data.get('liquidity_status', 'N/A'), data.get('mint_status', 'N/A'))
...
async def resume_after(minutes: int, admin_id: int):
    ...
    if current_status == 'paused':
        ...
        try:
            await retry_telethon_call(bot_client.send_message(admin_id, "▶️ Resumed after pause."))
        except Exception as e:
...
async def correct_last_announcement():
    ...
    if not await user_client.is_user_authorized():
         logger.warning("⚠️ User client not authorized. Cannot correct last announcement.")
         return
    ...
    for ch in targets:
        try:
            ...
            logger.error(f"Error correcting last announcement in channel {ch['channel_id']}: {e}", exc_info=True)
...
async def check_bot_admin() -> bool:
    ...
    for ch in target_channels:
        ...
        try:
            ...
            logger.error(f"Error checking bot admin status in channel {target_channel_id} ({ch.get('title', 'N/A')}): {e}", exc_info=True)
            is_admin_in_all_targets = False
        ...
        def ping():
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"⚠️ Self-ping returned status code {response.status_code}")
