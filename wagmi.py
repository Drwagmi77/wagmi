...
def get_channels_sync(channel_type): 
    ...
    try: 
        conn = get_connection() 
        with conn.cursor(cursor_factory=RealDictCursor) as cur: 
            cur.execute("SELECT * FROM channels WHERE channel_type = %s", 
(channel_type,)) 
            return cur.fetchall() 
    except Exception as e: 
...
def remove_channel_sync(channel_id, channel_type): 
    ...
    try: 
        conn = get_connection() 
        with conn.cursor() as cur: 
            cur.execute("DELETE FROM channels WHERE channel_id = %s AND channel_type 
= %s", 
...
def is_contract_processed_sync(contract_address): 
    ...
    try: 
        conn = get_connection() 
        with conn.cursor() as cur: 
            cur.execute("SELECT 1 FROM processed_contracts WHERE contract_address = 
%s", 
                        (contract_address,)) 
            return cur.fetchone() is not None 
    except Exception as e: 
...
def get_token_mapping_sync(token_name): 
    ...
    try: 
        conn = get_connection() 
        with conn.cursor(cursor_factory=RealDictCursor) as cur: 
            cur.execute("SELECT * FROM token_mappings WHERE token_name = %s", 
(token_name,)) 
            row = cur.fetchone() 
...
def add_token_mapping_sync(token_name, contract_address, 
announcement_message_id=None): 
    ...
    try: 
        ...
        with conn.cursor() as cur: 
            cur.execute(""" 
                INSERT INTO token_mappings (token_name, contract_address, 
announcement_message_id) 
                VALUES (%s, %s, %s) 
                ON CONFLICT (token_name) DO UPDATE 
                  SET contract_address = %s, 
                      announcement_message_id = COALESCE(%s, 
token_mappings.announcement_message_id) 
            """, (token_name, contract_address, announcement_message_id, 
                  contract_address, announcement_message_id)) 
        conn.commit() 
        if cur.rowcount > 0: 
             logger.info(f"Token mapping added/updated for '{token_name}' -> 
{contract_address}.") 
    except Exception as e: 
...
def update_token_announcement_sync(token_name, announcement_message_id): 
    ...
    try: 
        ...
        conn.commit() 
        if cur.rowcount > 0: 
             logger.info(f"Updated announcement ID for '{token_name}' to 
{announcement_message_id}.") 
        else: 
             logger.warning(f"No token mapping found for '{token_name}' to update 
announcement ID.") 
    except Exception as e: 
...
def get_mapping_by_announcement_sync(announcement_message_id): 
    ...
    try: 
        conn = get_connection() 
        with conn.cursor(cursor_factory=RealDictCursor) as cur: 
            cur.execute("SELECT * FROM token_mappings WHERE 
announcement_message_id = %s", 
                        (announcement_message_id,)) 
            row = cur.fetchone() 
            return dict(row) if row else None 
    except Exception as e: 
         logger.error(f"Error getting mapping by announcement ID 
{announcement_message_id}: {e}") 
         return None 
    finally: 
        if conn: 
            conn.close() 
...
def get_bot_setting_sync(setting): 
    ...
    try: 
        conn = get_connection() 
        with conn.cursor(cursor_factory=RealDictCursor) as cur: 
            cur.execute("SELECT setting_value FROM bot_settings WHERE setting_key = %s", 
(setting,)) 
            row = cur.fetchone() 
...
async def add_admin(user_id, first_name, last_name="", lang="en", is_default=False): 
    await asyncio.to_thread(add_admin_sync, user_id, first_name, last_name, lang, 
is_default) 
...
async def add_token_mapping(token_name, contract_address, 
announcement_message_id=None): 
    await asyncio.to_thread(add_token_mapping_sync, token_name, contract_address, 
announcement_message_id) 
 
async def update_token_announcement(token_name, announcement_message_id): 
    await asyncio.to_thread(update_token_announcement_sync, token_name, 
announcement_message_id) 
 
async def get_mapping_by_announcement(announcement_message_id): 
    return await asyncio.to_thread(get_mapping_by_announcement_sync, 
announcement_message_id) 
...
DEFAULT_BOT_SETTINGS = { 
    "bot_status": "running", 
    "custom_gif": 
"https://dl.dropbox.com/scl/fi/u6r3x30cno1ebmvbpu5k1/video.mp4?rlkey=ytfk8qkdpwwm3je6
 hjcqgd89s&st=vxjkqe6c?dl=1" 
} 
...
logger.info("
 🔥
 Logging setup complete. Bot is starting...") 
...
async def retry_telethon_call(coro, max_retries=5, base_delay=1.0): 
    for i in range(max_retries): 
        try: 
            ...
            logger.warning(f"Retry attempt {i+1}/{max_retries} for Telethon call due to database 
locked: {e}") 
            if i < max_retries - 1: 
...
def parse_tff_output(text: str) -> dict: 
    data = {} 
    data["mint_status"]      = (re.search(r"
 🌿
 \s*Mint:\s*(\w+)", text) or [None, "N/A"])[1] 
    data["liquidity_status"] = (re.search(r"Liq:\s*\$?([\d\.,KkMmBb]+)", text) or [None, "N/A"])[1] 
    data["market_cap"]       = (re.search(r"MC:\s*\$?([\d\.,KkMmBb]+)", text) or [None, 
"N/A"])[1] 
    logger.debug("
 ✅
 Parsed TTF output.") 
...
�
 \n\n" 
        f"
 💵
 *{token_name.upper()}* Market Cap: {new_mc} 
�
�
 \n" 
        f"
...
def build_announcement_buttons(contract): 
    return [ 
        [Button.url("
 📈
 Chart", f"https://dexscreener.com/solana/{contract}"), 
         Button.url("
 🛡
 Trojan", f"https://t.me/solana_trojanbot?start=r-ttf-{contract}")], 
        [Button.url("
 🐉
 Soul", f"https://t.me/soul_sniper_bot?start=4U4QhnwlCBxS_{contract}"), 
         Button.url("
 🤖
 MEVX", f"https://t.me/MevxTradingBot?start={contract}")], 
        [Button.url("
 📊
 Algora", f"https://t.me/algoratradingbot?start=r-tff-{contract}")], 
        [Button.url("
 🚀
 Trojan N", 
f"https://t.me/nestor_trojanbot?start=r-shielzuknf5b-{contract}"), 
         Button.url("
 🔗
 GMGN", 
f"https://t.me/GMGN_sol03_bot?start=CcJ5M3wBy35JHLp4csmFF8QyxdeHuKasPqKQeFa1
 TzLC")] 
    ] 
...
async def login(): 
    if request.method == 'POST': 
        ...
        try: 
            ...
            logger.info(f"
 ➡
 Sent login code request to {phone}") 
            return redirect('/submit-code') 
        except Exception as e: 
            logger.error(f"
 ❌
 Error sending login code to {phone}: {e}") 
...
async def submit_code(): 
    ...
    if request.method == 'POST': 
        ...
        try: 
            ...
            logger.info(f"
 ✅
 Logged in user-client for {phone}") 
            session.pop('phone', None) 
            return "<p>Login successful! You can close this tab.</p>" 
        except Exception as e: 
            logger.error(f"
 ❌
 Login failed for {phone}: {e}") 
...
buttons=kb, link_preview=False) 
 ...
 Bot stopped') 
            return await event.edit("
 🛑
 *Bot has been shut down.*", buttons=[[Button.inline("
 🔄
 
Restart Bot (set to running)", b"admin_start")], [Button.inline("
 🔙
 Back", b"admin_home")]], 
link_preview=False) 
        if data == 'admin_admins': 
            ...
            kb = [ 
                [Button.inline("
 ➕
 Add Admin", b"admin_add_admin")], 
            ] 
            removable_admins = {aid: info for aid, info in admins.items() if aid != 
DEFAULT_ADMIN_ID and not info.get("is_default")} 
            if removable_admins: 
                 kb.append([Button.inline("
 🗑
 Remove Admin", b"admin_show_remove_admins")]) 
            kb.append([Button.inline("
 🔙
 Back", b"admin_home")]) 
            return await event.edit("
 👤
 *Manage Admins*", buttons=kb, link_preview=False) 
            ...
            for aid, info in admins.items(): 
                if aid != DEFAULT_ADMIN_ID and not info.get("is_default"): 
                    kb.append([Button.inline(f"{info.get('first_name', 'N/A')} ({aid})", b"noop"), 
                               Button.inline("
 ❌
 Remove", f"remove_admin:{aid}".encode()) 
                              ]) 
            kb.append([Button.inline("
 🔙
 Back", b"admin_admins")]) 
            if not removable_admins: 
                 return await event.edit("
 🗑
 *No more removable admins.*", 
buttons=[[Button.inline("
 🔙
 Back", b"admin_admins")]], link_preview=False) 
            else: 
                 return await event.edit("
 🗑
 *Select Admin to Remove*", buttons=kb, 
link_preview=False) 
            ...
            for admin_id, info in admins.items(): 
                 if admin_id != DEFAULT_ADMIN_ID and not info.get("is_default"): 
                      kb.append([Button.inline(f"{info.get('first_name', 'N/A')} ({admin_id})", b"noop"), 
                                Button.inline("
 ❌
 Remove", f"remove_admin:{admin_id}".encode()) 
                               ]) 
            kb.append([Button.inline("
 🔙
 Back", b"admin_admins")]) 
...
link_preview=False) 
        ...
        if data == 'admin_targets': 
            kb = [ 
                [Button.inline("
 ➕
 Add Target", b"admin_add_target")], 
            ] 
            targets = await get_channels('target') 
            if targets: 
                 kb.append([Button.inline("
 🗑
 Remove Target", b"admin_show_remove_targets")]) 
            kb.append([Button.inline("
 🔙
 Back", b"admin_home")]) 
            return await event.edit("
 📺
 *Manage Target Channels*", buttons=kb, 
link_preview=False) 
        if data == 'admin_show_remove_targets': 
             ...
             kb = [ 
                 [Button.inline(ch.get('title', 'N/A'), b"noop"), 
                  Button.inline("
 ❌
 Remove", f"remove_target:{ch['channel_id']}".encode())] 
                 for ch in targets 
             ] 
             kb.append([Button.inline("
 🔙
 Back", b"admin_targets")]) 
             return await event.edit("
 🗑
 *Select Target Channel to Remove*", buttons=kb, 
link_preview=False) 
        if data == 'admin_add_target': 
            pending_input[uid] = {'action': 'confirm_add_target'} 
            return await event.edit("
 ➕
 *Add Target Channel*\n\nSend me the channel ID (e.g., 
`-1001234567890`) or @username to add:", buttons=[[Button.inline("
 🔙
 Back", 
b"admin_targets")]], link_preview=False) 
        if data.startswith('remove_target:'): 
            tid = int(data.split(':')[1]) 
            await remove_channel(tid, "target") 
            await event.answer("
 ✅
 Target channel removed", alert=True) 
            targets = await get_channels('target') 
            kb = [ 
                 [Button.inline(ch.get('title', 'N/A'), b"noop"), 
                  Button.inline("
 ...
 Back", b"admin_targets")]) 
            if not targets: 
                 return await event.edit("
 🗑
 *No more target channels.*", 
buttons=[[Button.inline("
 🔙
 Back", b"admin_targets")]], link_preview=False) 
            else: 
                 return await event.edit("
 🗑
 *Select Target Channel to Remove*", buttons=kb, 
link_preview=False) 
 ...
 🗑
 Remove Source", 
b"admin_show_remove_sources")]) 
            kb.append([Button.inline("
 🔙
 Back", b"admin_home")]) 
            return await event.edit("
 📡
 *Manage Source Channels*", buttons=kb, 
link_preview=False) 
        if data == 'admin_show_remove_sources': 
             ...
             kb = [ 
                 [Button.inline(ch.get('title', 'N/A'), b"noop"), 
                  Button.inline("
 ❌
 Remove", f"remove_source:{ch['channel_id']}".encode())] 
                 for ch in sources 
             ] 
             kb.append([Button.inline("
 🔙
 Back", b"admin_sources")]) 
             return await event.edit("
 🗑
 *Select Source Channel to Remove*", buttons=kb, 
link_preview=False) 
...
`-1001234567890`) or @username to add:", buttons=[[Button.inline("
 🔙
 Back", 
b"admin_sources")]], link_preview=False) 
        if data.startswith('remove_source:'): 
            sid = int(data.split(':')[1]) 
            await remove_channel(sid, "source") 
            await event.answer("
 ✅
 Source channel removed", alert=True) 
            sources = await get_channels('source') 
            kb = [ 
                 [Button.inline(ch.get('title', 'N/A'), b"noop"), 
                  Button.inline("
 ...
 Back", b"admin_sources")]) 
            if not sources: 
                 return await event.edit("
 🗑
 *No more source channels.*", 
buttons=[[Button.inline("
 🔙
 Back", b"admin_sources")]], link_preview=False) 
            else: 
                 return await event.edit("
 🗑
 *Select Source Channel to Remove*", buttons=kb, 
...
buttons=[[Button.inline("
 🔙
 Back", b"admin_home")]], link_preview=False) 
...
link_preview=False) 
    ...
    await event.answer("
 ✅
 Done" if not event.answered else "") 
        ...
        try: 
            if act == 'pause': 
                try: 
                    ...
                    await event.reply("
 ⚠
 Please send a valid positive number of minutes.") 
                    pending_input[uid] = {'action': 'pause'} 
                    return 
                await set_bot_setting("bot_status", "paused") 
                await event.reply(f"
 ⏸
 Paused for {m} minutes.") 
                asyncio.create_task(resume_after(m, uid)) 
                await retry_telethon_call(bot_client.send_message(uid, await 
get_admin_dashboard(), buttons=build_admin_keyboard(), link_preview=False)) 
                return 
            ...
            if act == 'confirm_add_admin': 
                try: 
                    ...
                    await event.reply("
 ⚠
 Invalid user ID. Please send a positive integer ID.") 
                    pending_input[uid] = {'action': 'confirm_add_admin'} 
                    return 
                ...
                if new_id in current_admins: 
                    await event.reply(f"
 ℹ
 User ID {new_id} is already an admin.") 
                else: 
            ...
            if act == 'confirm_add_target': 
                ...
                try: 
                    ...
                    try: 
                        ...
                        await bot_client(GetParticipantRequest(channel=channel_id, 
participant=me.id)) 
                    except Exception: 
                         await event.reply("
 ❌
 Bot is not in that channel or doesn't have access. 
Make sure the bot is added.") 
                         pending_input[uid] = {'action': 'confirm_add_target'} 
                         return 
                    ...
                    await event.reply(f"
 ✅
 Target channel {channel_title} ({channel_id}) added.") 
                    await retry_telethon_call(bot_client.send_message(uid, await 
get_admin_dashboard(), buttons=build_admin_keyboard(), link_preview=False)) 
                    return 
                    ...
                    await event.reply(f"
 ❌
 Could not add target channel '{channel_input}'. Error: 
{e}") 
                    pending_input[uid] = {'action': 'confirm_add_target'} 
                    return 
            ...
            if act == 'confirm_add_source': 
                ...
                try: 
                    if not await user_client.is_user_authorized(): 
                         await event.reply("
 ❌
 User client not authorized. Cannot verify source 
channel access.") 
                         pending_input[uid] = {'action': 'confirm_add_source'} 
                         return 
                    ...
                    try: 
                        ...
                        await user_client(GetParticipantRequest(channel=channel_id, 
participant=me_user.id)) 
                    except Exception: 
                         await event.reply("
 ❌
 Your user account is not in that source channel or it's 
not accessible.") 
                         pending_input[uid] = {'action': 'confirm_add_source'} 
                         return 
                    ...
                    await event.reply(f"
 ✅
 Source channel {channel_title} ({channel_id}) added.") 
                    await retry_telethon_call(bot_client.send_message(uid, await 
get_admin_dashboard(), buttons=build_admin_keyboard(), link_preview=False)) 
                    return 
                    ...
                    await event.reply(f"
 ❌
 Could not add source channel '{channel_input}'. Error: 
{e}") 
                    pending_input[uid] = {'action': 'confirm_add_source'} 
                    return 
            ...
            if act == 'confirm_update_gif': 
                ...
                if not link.startswith(('http://', 'https://')): 
                    await event.reply("
 ⚠
 Invalid URL format. Please send a valid HTTP or HTTPS 
URL.") 
                    pending_input[uid] = {'action': 'confirm_update_gif'} 
                    return 
...
get_admin_dashboard(), buttons=build_admin_keyboard(), link_preview=False)) 
 ...
 An unexpected error occurred while processing your input.") 
            ...
            try: 
                 await retry_telethon_call(bot_client.send_message(uid, "
 ⚠
 An error occurred. 
Returning to dashboard.", buttons=build_admin_keyboard(), link_preview=False)) 
            except Exception: 
                 pass 
...
@user_client.on(events.NewMessage(incoming=True, chats=[c['channel_id'] for c in 
get_channels_sync('source')])) 
async def channel_handler(event): 
    ...
    if bot_status != 'running': 
        logger.info(f"Bot status is '{bot_status}'. Skipping message processing for {message_id} 
in {chat_id}.") 
        return 
    ...
    logger.info(f"
 📥
 Processing message {message_id} at {now} from chat {chat_id}: 
{txt[:100]}...") 
 
 
    update_pattern_check = 
re.compile(r"MC:\s*\$?[\d\.,KkMmBb]+\s*(?:->|[-–>→])\s*\$?([\d\.,KkMmBb]+)", 
re.IGNORECASE) 
    ...
    if update_pattern_check.search(txt): 
        ...
        if token_sym == "unknown": 
            logger.warning(f"Could not extract token symbol from update message 
{message_id}. Skipping update processing.") 
            return 
        ...
        if mapping: 
            ...
            logger.info(f"Found mapping for token '{token_sym}': CA={contract_for_button}, 
ReplyMsgID={announcement_to_reply_to} for message {message_id}") 
        else: 
            logger.warning(f"No mapping found for token symbol: '{token_sym}' in update 
message {message_id}. Cannot reply to initial announcement.") 
...
re.IGNORECASE) 
        ...
        if mc_match_obj: 
            ...
            logger.debug(f"Extracted new MC: {new_mc} from message {message_id}.") 
             ...
             logger.warning(f"Could not extract new MC using primary regex for update message 
{message_id}: {txt[:100]}...") 
             simple_mc_search = 
re.findall(r"MC:\s*\$?(?:[\d\.,KkMmBb]+\s*(?:->|[-–>→])\s*\$?)?([\d\.,KkMmBb]+)", txt, 
re.IGNORECASE) 
             if simple_mc_search: 
                 new_mc = simple_mc_search[-1] 
                 logger.debug(f"Extracted new MC (fallback): {new_mc} from message 
{message_id}.") 
             else: 
                logger.warning(f"Could not extract *any* MC value for update message 
{message_id}: {txt[:100]}...") 
        ...
        for target_channel_info in target_channels: 
            ...
            try: 
                logger.info(f"Sending update for '{token_sym}' to target channel ID: 
{target_channel_id} (replying to {announcement_to_reply_to}).") 
 ...
 🔗
 Don't Miss Out", 
f"https://t.me/solana_trojanbot?start=r-ttf-{contract_for_button}")]] 
                )) 
                logger.info(f"Update sent successfully to {target_channel_id}.") 
            except Exception as e: 
                logger.error(f"Error sending update to target {target_channel_id} for message 
{message_id}: {e}", exc_info=True) 
    ...
    if not contract: 
        logger.info(f"No contract address found in message {message_id}. Skipping new call 
processing.") 
        return 
 
    if await is_contract_processed(contract): 
        logger.info(f"Contract {contract} from message {message_id} already processed. 
Skipping.") 
        return 
    ...
    logger.info(f"
 ➡
 Sending contract {contract} to @ttfbotbot at {now} for message 
{message_id}.") 
    ttf_response = None 
    try: 
        ...
        if not await user_client.is_user_authorized(): 
            logger.warning("
 ⚠
 User client not authorized. TTF bot interaction skipped for new 
call.") 
            return 
        ...
        async with user_client.conversation('@ttfbotbot', timeout=90) as conv: 
            ...
            logger.info(f"
 ⬅
 Received response from @ttfbotbot (Msg ID: {ttf_response.id}) for 
message {message_id}.") 
 
    except asyncio.TimeoutError: 
        logger.warning(f"
 ⚠
 TTF bot conversation timed out for contract {contract} from 
message {message_id}.") 
        await retry_telethon_call(bot_client.send_message(DEFAULT_ADMIN_ID, f"
 ⚠
 TTF bot 
timed out for contract: `{contract}` (from message {message_id} in {chat_id}).")) 
        return 
 
    except Exception as e: 
        logger.error(f"
 ⚠
 TTF bot error for contract {contract} (from message {message_id}): 
{e}", exc_info=True) 
        await retry_telethon_call(bot_client.send_message(DEFAULT_ADMIN_ID, f"
 ❌
 TTF bot 
error for contract `{contract}` (from message {message_id} in {chat_id}): {e}")) 
        return 
 
 
    if ttf_response and ttf_response.raw_text: 
        logger.info(f"Parsing TTF bot output for contract {contract}: 
{ttf_response.raw_text[:100]}...") 
        data = parse_tff_output(ttf_response.raw_text) 
 
        token_name = extract_token_name_from_source(txt) 
        if token_name == "unknown": 
            logger.warning(f"Could not extract token name from source message {message_id} 
for contract {contract}. Using 'UNKNOWN'.") 
            token_name = "UNKNOWN" 
...
data.get('liquidity_status', 'N/A'), data.get('mint_status', 'N/A')) 
        ...
        for target_channel_info in target_channels: 
            target_channel_id = target_channel_info["channel_id"] 
            try: 
                logger.info(f"Sending new call announcement for '{token_name}' ({contract}) to 
target channel ID: {target_channel_id}.") 
                ...
                logger.info(f"New announcement sent to {target_channel_id}, message_id: 
{msg.id}.") 
                ...
                logger.error(f"Error sending new call announcement to target {target_channel_id} 
for contract {contract}: {e}", exc_info=True) 
 
        if announcement_id is not None: 
             await add_token_mapping(token_name.lower(), contract, announcement_id) 
             logger.info(f"Recorded mapping for '{token_name}' -> {contract} with announcement 
ID {announcement_id}.") 
        else: 
             await add_token_mapping(token_name.lower(), contract, None) 
             logger.warning(f"Failed to send announcement to any target channels for 
'{token_name}' ({contract}). Mapping stored without announcement ID.") 
 
 
    else: 
        logger.warning(f"TTF bot did not return a message or message was empty for contract 
{contract} from message {message_id}. Cannot announce.") 
        await retry_telethon_call(bot_client.send_message(DEFAULT_ADMIN_ID, f"
 ⚠
 TTF bot 
returned empty message for contract: `{contract}` (from message {message_id} in {chat_id}). 
Cannot announce.")) 
...
async def resume_after(minutes: int, admin_id: int): 
    ...
    if current_status == 'paused': 
        ...
        try: 
            await retry_telethon_call(bot_client.send_message(admin_id, "
 ▶
 Resumed after 
pause.")) 
        except Exception as e: 
        ...
        logger.info(f"Bot status changed from 'paused' to '{current_status}' during pause period. 
Not automatically resuming.") 
...
async def correct_last_announcement(): 
    ...
    if not await user_client.is_user_authorized(): 
         logger.warning("
 ⚠
 User client not authorized. Cannot correct last announcement.") 
         return 
    ...
    for ch in targets: 
        try: 
            ...
            last_msgs = await retry_telethon_call(user_client.get_messages(ch["channel_id"], 
limit=1)) 
            if not last_msgs: 
            ...
            if not text: 
                 logger.debug(f"Last message in target channel {ch['channel_id']} is empty. 
Skipping correction.") 
                 continue 
 
            extracted_token = extract_token_name_from_source(text) 
            if extracted_token == "unknown": 
                 logger.debug(f"Could not extract token from last message {last_msg.id} in channel 
{ch['channel_id']}. Skipping correction.") 
                 continue 
            ...
            if mapping_by_id: 
                ...
                if old_token and contract_address and extracted_token.lower() != 
old_token.lower(): 
                    await add_token_mapping(extracted_token.lower(), contract_address, 
last_msg.id) 
                    logger.info("Corrected token mapping for message id %s in channel %s: '%s' -> 
'%s' (CA: %s)", 
                                last_msg.id, ch["channel_id"], old_token, extracted_token, 
contract_address) 
                elif old_token and contract_address: 
                     logger.debug(f"Mapping for message ID {last_msg.id} in channel 
{ch['channel_id']} is already correct ('{old_token}').") 
                else: 
                     logger.warning(f"Mapping found by ID {last_msg.id} but missing token_name or 
contract_address in DB for channel {ch['channel_id']}.") 
 
            else: 
                if "New 
�
�
 GEM Landed!" in text and extract_contract(text): 
                     existing_mapping_by_token = await 
get_token_mapping(extracted_token.lower()) 
                     if existing_mapping_by_token and 
existing_mapping_by_token.get("announcement_message_id") is None: 
                     ...
                     logger.debug(f"No existing mapping found by announcement ID {last_msg.id} 
and message doesn't look like a new announcement in channel {ch['channel_id']}.") 
...
ChannelParticipantCreator)): 
                logger.error(f"
 ❌
 Bot lacks admin rights in target channel: {target_channel_id} 
({ch.get('title', 'N/A')}). Required for posting.") 
                is_admin_in_all_targets = False 
            else: 
                 logger.info(f"
 ✅
 Bot has admin rights in target channel: {target_channel_id} 
({ch.get('title', 'N/A')}).") 
 
        except Exception as e: 
            logger.error(f"Error checking bot admin status in channel {target_channel_id} 
({ch.get('title', 'N/A')}): {e}", exc_info=True) 
            is_admin_in_all_targets = False 
...
async def get_admin_dashboard(): 
    ...
    try: 
        aff_response = await loop.run_in_executor(None, requests.get, 
"https://www.affirmations.dev") 
    ...
    try: 
        quote_response = await loop.run_in_executor(None, requests.get, 
...
�
 \n\n" 
        f"
 🤖
 *Bot Status:* `{bot_status.capitalize()}`\n\n" 
        f"
 💖
 *Affirmation:* {aff}\n" 
        f"
...
def build_admin_keyboard(): 
    return [ 
        [Button.inline("
 ▶
 Start Bot", b"admin_start"), 
         Button.inline("
 ⏸
 Pause Bot", b"admin_pause"), 
         Button.inline("
 🛑
 Stop Bot", b"admin_stop")], 
        [Button.inline("
 👤
 Admins", b"admin_admins"), 
         Button.inline("
 📺
 Targets", b"admin_targets"), 
         Button.inline("
 📡
 Sources", b"admin_sources")], 
        [Button.inline("
 🎬
 Update GIF", b"admin_update_gif")] 
    ] 
...
async def main(): 
    ...
    logger.info("
 🤖
 Bot client started and connected.") 
 
    await user_client.connect() 
    if await user_client.is_user_authorized(): 
        logger.info("
 👤
 User client authorized.") 
        asyncio.create_task(correct_last_announcement()) 
        logger.info("Started background task: correct_last_announcement.") 
    else: 
        logger.warning("
 ⚠
 User client not yet authorized; please visit the /login page via web.") 
        logger.warning("
 ⚠
 Bot might not function correctly without user client authorized (TTF 
interaction, source channel access).") 
 
 
    if not await check_bot_admin(): 
        logger.error("
 ❌
 Bot lacks admin rights in one or more target channels. Posting might 
fail.") 
    else: 
        logger.info("
 ✅
 Bot has admin rights in all configured target channels.") 
...
if __name__ == '__main__': 
 ...
 Self-ping OK") 
                    ...
                    logger.warning(f"
 ⚠
 Self-ping returned status code {response.status_code}") 
            except requests.exceptions.ConnectionError as e: 
                 logger.error(f"
 ❌
 Self-ping failed: ConnectionError - {e}") 
            except Exception as e: 
                logger.error(f"
 ❌
 Self-ping failed: {e}") 
