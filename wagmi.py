...
TARGET_CHANNEL_TITLE = "Wagmi Vip ☢️"
...
async def message_handler(event):
    ...
    # Filter out messages that appear to be updates
    text_lower = text.lower()
    if "market cap:" in text_lower and ("profit" in text_lower or "x" in text_lower):
    ...
    # Extract original market cap from the announcement (if available)
    ...
    # Extract original token name from a line like "Ass Coin - $Ass"
    orig_token_match = re.search(r"\n\s*(.+?)\s*-\s*\$(\w+)", text)
    if orig_token_match:
        token_full = orig_token_match.group(1).strip()
        token_symbol = orig_token_match.group(2).strip().lower()  # use symbol as key
    else:
        token_full = None
        token_symbol = None
    ...
    # Use original values if available; otherwise fall back to TFF bot response
    ...
    # Use token symbol as key if available; otherwise default to lowercased token_name.
...
async def token_update_handler(event):
    ...
    text_lower = text.lower()
    # Check for update message by looking for '$' and 'market cap:'
    if "$" in text and "market cap:" in text_lower:
        token_match = re.search(r"\$(\w+)", text)
        token_symbol = token_match.group(1).lower().strip() if token_match else None

        # If token symbol isn't found, default to "unknown"
        if not token_symbol:
            logger.info("Token symbol not extracted via regex; defaulting to 'unknown'.")
            ...
            if token_symbol not in token_mapping:
                ...
                contract = token_mapping[token_symbol]

        logger.info("Update message token extracted: %s", token_symbol)
        ...
        if mc_match:
            ...
            logger.info("Market cap change not found in update message for '%s'.", token_symbol)

        profit_match = re.search(r"(\d+)%", text)
        profit_text = f"{profit_match.group(1)}%" if profit_match else "PROFITS"
        if not profit_match:
            logger.info("Profit percentage not found in update message for '%s'.", token_symbol)
        ...
        if trojan_match:
            ...
            logger.info("Using fallback Trojan URL for '%s'.", token_symbol)
        ...
        try:
            # Send a threaded reply using reply_to parameter
            await bot_client.send_file(
                event.chat_id,
                file=custom_gif_path,
        ...
        logger.info("Update message did not meet conditions; skipping update.")
